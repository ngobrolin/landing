# OpenAI Whisper Transcribe Script Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Create `scripts/transcribe-openai.js` that transcribes podcast episodes using the OpenAI Whisper API (or any compatible endpoint) instead of the local whisper.cpp CLI.

**Architecture:** New standalone script mirroring `scripts/transcribe.js` — downloads mp3 via yt-dlp, calls the OpenAI audio transcriptions API with `verbose_json` response format, transforms segments to match the existing transcript JSON schema, and saves to `src/data/transcripts/`. Supports OpenAI-compatible endpoints via `--base-url`.

**Tech Stack:** Node.js ESM, `openai` npm package, yt-dlp CLI
...
**Step 1: Install the openai package as a dev dependency**

```bash
npm install --save-dev openai
```

Expected: `openai` appears in `devDependencies` in `package.json`.
...
```bash
node -e "const p = JSON.parse(require('fs').readFileSync('package.json','utf-8')); console.log(p.scripts['transcribe:openai'], p.devDependencies['openai'])"
```

Expected: prints the script value and the openai version string.

**Step 4: Commit**

```bash
git add package.json package-lock.json
git commit -m "feat: add openai dev dependency and transcribe:openai npm script"
```

---

### Task 2: Create scripts/transcribe-openai.js

**Files:**
- Create: `scripts/transcribe-openai.js`
...
```bash
node --input-type=module < scripts/transcribe-openai.js 2>&1 | head -5
```

Expected: the script starts, hits the missing `OPENAI_API_KEY` check, and prints:
```
Error: OPENAI_API_KEY environment variable is required.
```
(Exit code 1 — this confirms the file parses and runs correctly.)

**Step 3: Commit**

```bash
git add scripts/transcribe-openai.js
git commit -m "feat: add transcribe-openai.js using OpenAI Whisper API"
```

---

### Task 3: Smoke test with a real episode

> This task requires `OPENAI_API_KEY` to be set and yt-dlp available.

**Step 1: Pick an existing episode's videoId to re-transcribe (so there's a baseline to compare)**

```bash
ls src/data/transcripts/ | head -1
```

Note the videoId (e.g., `0o-PcX6pR2E`).

**Step 2: Run the script against that single episode**

```bash
OPENAI_API_KEY=your_key_here node scripts/transcribe-openai.js 0o-PcX6pR2E
```

Or using npm:
```bash
OPENAI_API_KEY=your_key_here npm run transcribe:openai -- 0o-PcX6pR2E
```

Expected output:
```
Processing 1 episode(s)...

[0o-PcX6pR2E] <episode title>
  Downloading audio for 0o-PcX6pR2E...
  Transcribing 0o-PcX6pR2E via OpenAI API (model: whisper-1)...
  Saved transcript: .../src/data/transcripts/0o-PcX6pR2E.json
...
**Step 3: Verify the output JSON has the correct shape**

```bash
node -e "
const t = JSON.parse(require('fs').readFileSync('src/data/transcripts/0o-PcX6pR2E.json', 'utf-8'));
...
Expected: all fields present, segment has `start`, `end`, `text`, `fullText` is non-empty.

**Step 4: Test the missing-API-key guard**

```bash
node scripts/transcribe-openai.js 2>&1
```
...
**Step 5: Test the --base-url flag (dry run, invalid URL to check arg parsing)**

```bash
OPENAI_API_KEY=test node scripts/transcribe-openai.js --base-url https://example.com/v1 --limit 0 --all 2>&1 | head -3
```

Expected: processes 0 episodes (limit 0 gets clamped by the "toProcess.length === 0" branch printing "All episodes already have transcripts!" is fine — we just want to confirm the flag parses without crashing). Actually --limit 0 triggers the error path. Use a valid limit with --missing if all are done. The goal is just to confirm --base-url doesn't crash arg parsing.
...
# With Groq-compatible endpoint
OPENAI_API_KEY=gsk_... npm run transcribe:openai -- --base-url https://api.groq.com/openai/v1 --model whisper-large-v3 --missing

# Limit to 3 episodes using Safari cookies
OPENAI_API_KEY=sk-... npm run transcribe:openai -- --missing --limit 3 --browser safari
```
