...
- If the finding has no value-specific command, such as `side-tab`, scope that one rule to the file: `ignore-value <id> "*" --file <path>`. Run `pnpm dlx impeccable detect <path>` first to see what actually fires there.
